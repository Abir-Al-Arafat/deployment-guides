...
This guide details how to map your subdomains (`api.domain.app` and `dashboard.domain.app`) to your EC2 instance using Nginx and secure them with free SSL certificates via Let's Encrypt (Certbot).
...
server {
    listen 80;
    server_name dashboard.domain.app;
...
Once DNS propagation completes for `api.domain.app` and `dashboard.domain.app`, install Certbot to secure your traffic with free SSL certificates:
...
sudo certbot --nginx -d api.domain.app -d dashboard.domain.app
```
